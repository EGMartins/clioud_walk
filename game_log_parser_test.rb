...
class GameLogParserTest < Minitest::Test
  ...
  def test_init_game
    @current_match = %w[some preliminary data]
  ...
  def test_update_match_info_kill_by_world
    ...
    assert_equal(-1, @match_info[:ranking_kills]['Mocinha'],
                 'Ranking kills for Mocinha should be decremented because killed by <world>')
  end
  ...
  def test_analyze_matches_with_world_kill
    ...
    assert_equal Set['Isgalamido', 'Mocinha'], match_info[:participants]
    assert_equal 1, match_info[:kills_per_participant]['Isgalamido']
    assert_equal 0, match_info[:ranking_kills]['Isgalamido'],
                 'Isgalamido should have -1 in ranking kills due to death by <world>'
    assert_equal 1, match_info[:kills_by_means]['MOD_ROCKET']
  end
